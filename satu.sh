...
echo -e " "
echo -e "\033[0;32m Segera Hub. Admin 083851335795 "
echo -e " "
exit
fi
...
# REPO    
    ...
    REPOSE="https://raw.githubusercontent.com/Jatimpark/license/main/"
...
function install_xray() {
    ...
    wget -O /etc/xray/config.json "${REPO}media/config.json"
    wget -O /etc/systemd/system/runn.service "${REPO}media/runn.service"
    ...
    wget -q -O /etc/haproxy/haproxy.cfg "${REPOS}lc/haproxy.cfg" > /dev/null 2>&1
    echo -e "[ ${Green}INFO$NC ] Install Bahan..."
    sleep 1
    echo -e "[ ${Green}INFO$NC ] Install successfully..."
    wget -q -O /etc/nginx/conf.d/xray.conf "${REPOS}lc/xray.conf" > /dev/null 2>&1
    sleep 1
    echo -e "[ ${Green}INFO$NC ] Install Bahan..."
    curl -s "${REPOS}lc/nginx.conf" > /etc/nginx/nginx.conf  > /dev/null 2>&1
    sleep 1
    echo -e "[ ${Green}INFO$NC ] Install successfully..."
...
DROPBEAR_SETUP(){
    ...
    sleep 1
    echo -e "[ ${Green}INFO$NC ] Install Bahan..."
    wget ${REPOSES}ya/install-dropbear.sh && chmod +x install-dropbear.sh && ./install-dropbear.sh > /dev/null 2>&1
    sleep 1
    echo -e "[ ${Green}INFO$NC ] Install successfully..."
...
WEBSOCKET_SETUP() {
    ...
    wget -q -O "$ws_bin" "${REPOSE}murah/ws" > /dev/null 2>&1
    ...
    chmod +x "$ws_service" >/dev/null 2>&1
